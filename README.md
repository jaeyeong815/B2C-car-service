...
https://github.com/WantedPreonboardingFE5team/pre-onboarding-7th-2-1-5/blob/b4b0cbf1fec50806f0c9dfb7efa8b4b84984e7af/src/components/Main/Cards/CardList.jsx#L18-L36

- 에러 내용 : `react-snap`은 지정한 페이지를 index.html로 랜더링 후 빌드 파일에 저장을 하는데 `useNavigate()` 를 사용함으로서 빌드 시 `react-snap`이 세부 페이지를 감지하지 못했습니다.
- 해결 : `useNavigate()` 를 쓰는 대신 각 리스트를 `<Link>`태그를 사용해서 `<meta>` 태그가 적용된 각 index.html 파일이 생성된 것을 확인하고 해결했습니다. 
